...
int osecpuVmStackFree(OsecpuVm *vm, int totalSize, int typ, int len);
...
// bitRが32以下であることとbitFが64以下であることも呼び出し元が保証する.
{
	...
	int f1Size = f1 * sizeof (double), bitSize = (r1 + f1) * sizeof (int);
	int totalSize, i, bit;
	StackHeader *psh;
	char *p;
	r1Size  = (r1Size  + 15) & -16;
...
int osecpuVmStackPull(OsecpuVm *vm, int r1, int bitR, int p1, int f1, int bitF)
{
	...
	for (i = 0; i < r1; i++) {
		vm->r[i] = psh->r0p[i];
		vm->bit[i] = psh->bitR0p[i];
	}
	for (i = 0; i < p1; i++)
		vm->p[i] = psh->p0p[i];
	for (i = 0; i < f1; i++) {
		vm->f[i] = psh->f0p[i];
		vm->bitF[i] = psh->bitF0p[i];
	}
	vm->p[0x30] = psh->p30;
